...
*Перехід з будь-якої іншої прошивки, включаючи офіційну 7.0, треба здійснювати через **Full Chip Ease***
...
*Якщо режим ```set inav_allow_gps_fix_estimation = ON```: відключення працює після того, я літак знайде супутники і зафіксує точку повернення, і лише в режимі ARM.*
