...
***На відміну від версіі з inav 6.0, ця версія включає естимацію не лише при втраті супутників, але й при апаратній проблемі GPS сенсору, що дозволяє його не встановлювати в "екстремальних" сетапах.***
...
Прошивка не змінилася, виправлення лише в конфігураторі 7.0.2:

- виправлено: не встановлюється Band L через конфігуратор Configuration/VTX, але через CMS menu, Programming, Adjustments - працює

- виправлено: не працює demo-режим
...
- built-in serial receiver support https://github.com/iNavFlight/inav-configurator/pull/1849
...
*За замовчування координати safehome - 0,0. Можна злетіти з відключеним GPS в режимі ON_SAFE_HOME, в цьому випадку літак буде вважати, що літає десь біля екватору і буде повертатись теж в точку 0,0. Однак, якщо літак отримає реальні координати з сенсору (і сенсор буде включений), він опиниться за сотні кілометрів від зафіксованої точки дому (0,0) та в разі RTH буде повертатись кудись на південь. В любому випадку, краще встановити **safehome 0** в якусь точку на північ від реальноі позиції зльоту, щоб літак повертався на північ.*
...
Якщо працює спуфінг, який зміщує координати, літак буде думати, що Groundspeed > 0. В цьому випадку autolaunch не буде включатись. В такому випадку треба примусово відключити GPS, щоб gorundspeed була 0 - тоді можна злетіти з autolaunch.
